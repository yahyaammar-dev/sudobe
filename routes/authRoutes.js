...
router.post('/login', async (req, res) => {
  try {
    ...
    if (!user) {
      return res.redirect(`${process.env.BASE_PATH || ''}/auth.html?status=error&message=Invalid credentials`);
    }
    ...
    res.redirect(`${process.env.BASE_PATH || ''}/api/content`);

    // Check if user has phone number for 2FA
    // if (!user.content.phone) {
    //   return res.redirect(`${process.env.BASE_PATH || ''}/auth.html?status=error&message=Phone number not configured for 2FA`);
    // }
    ...
    // return res.redirect(`${process.env.BASE_PATH || ''}/otp-verify.html?userId=${user.id}&channel=${["failed", "undelivered"].includes(statusCheck.status) ? "sms" : "whatsapp"}`);

    // } catch (otpError) {
    //   console.error('OTP send error:', otpError);
    //   return res.redirect(`${process.env.BASE_PATH || ''}/auth.html?status=error&message=Failed to send OTP`);
    // }

  } catch (error) {
    console.error('Login error:', error);
    res.redirect(`${process.env.BASE_PATH || ''}/auth.html?status=error&message=Internal server error`);
  }
});
...
router.post('/verify-otp', async (req, res) => {
  try {
    ...
    if (!userId || !otp) {
      return res.redirect(`${process.env.BASE_PATH || ''}/otp-verify.html?status=error&message=Missing required fields`);
    }
    ...
    if (!user) {
      return res.redirect(`${process.env.BASE_PATH || ''}/auth.html?status=error&message=User not found`);
    }

    // Check if OTP exists and hasn't expired
    if (!user.content.otp || !user.content.otpExpiry) {
      return res.redirect(`${process.env.BASE_PATH || ''}/auth.html?status=error&message=No OTP found. Please login again`);
    }
    ...
    if (now > otpExpiry) {
      ...
      return res.redirect(`${process.env.BASE_PATH || ''}/auth.html?status=error&message=OTP expired. Please login again`);
    }

    // Verify OTP
    if (parseInt(otp) !== parseInt(user.content.otp)) {
      return res.redirect(`${process.env.BASE_PATH || ''}/otp-verify.html?userId=${userId}&status=error&message=Invalid OTP`);
    }
...
router.post('/register', async (req, res) => {
  try {
    ...
    if (existing.results.length > 0) {
      return res.redirect(`${process.env.BASE_PATH || ''}/auth.html?status=error&message=Email already exists`);
    }
    ...
    res.redirect(`${process.env.BASE_PATH || ''}/auth.html?status=success&message=User registered successfully`);
...
router.get('/logout', (req, res) => {
  ...
  res.redirect(`${process.env.BASE_PATH || ''}/auth.html`);
});
